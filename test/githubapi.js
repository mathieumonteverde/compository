...
describe('GET repo', () => {
  beforeEach(() => {
    ...
    const succeededRepo = JSON.parse(fs.readFileSync(`${__dirname}/succeededRepoRequest.json`, 'utf8'));
    ...
    const failedRepo = JSON.parse(fs.readFileSync(`${__dirname}/failedRepoRequest.json`, 'utf8'));
    ...
    const exceededLimit = JSON.parse(fs.readFileSync(`${__dirname}/exceededLimitRequest.json`, 'utf8'));
  ...
  it('should return an error (Exceeded limit request)', (done) => {
    ...
    githubapi.getRepo('octokit', 'limit', (err, response) => {
      ...
      err.message.should.be.a('string');
      const error = err.message.startsWith('API rate limit exceeded');
...
describe('GET number of commits', () => {
  beforeEach(() => {
    // Succeeded repo request
    const succeededCommit = JSON.parse(fs.readFileSync(`${__dirname}/succeededCommitsRequest.json`, 'utf8'));
...
describe('GET number of contributors', () => {
  beforeEach(() => {
    // Succeeded repo request
    const succeededContributors = JSON.parse(fs.readFileSync(`${__dirname}/succeededContributorsRequest.json`, 'utf8'));
